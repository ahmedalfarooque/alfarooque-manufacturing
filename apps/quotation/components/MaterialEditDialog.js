  ...
  if (!form) {
    return (
      <Modal title={t('matdlg.title')} onClose={() => onDone({ action: 'cancel' })} wide>
        <div className="text-sm text-[#7C9296]">{err || t('shell.loading')}</div>
      </Modal>
    );
  }
  ...
  return (
    <Modal title={t('matdlg.title') + (row?.code ? ' — ' + row.code : '')} onClose={requestClose} wide>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="md:col-span-3 text-[12px] text-[#7C9296]">{t('materials.priceEditNote')}</div>
        ...
        <div className="md:col-span-3 grid grid-cols-1 md:grid-cols-3 gap-3 pt-1">
          <div className="flex flex-col gap-1">
            ...
            <span className="text-[11px] text-[#7C9296] leading-snug">{t('matdlg.saveHint')}</span>
          </div>
          <div className="flex flex-col gap-1">
            <Button variant="ghost" disabled={busy || !(form.name_en || form.name_ar)} onClick={doSaveAsNew}>
              {t('matdlg.saveAsNew')}
            </Button>
            <span className="text-[11px] text-[#7C9296] leading-snug">{t('matdlg.saveAsNewHint')}</span>
          </div>
          <div className="flex flex-col gap-1">
            <Button variant="ghost" disabled={busy} onClick={doClose}>{t('common.close')}</Button>
            <span className="text-[11px] text-[#7C9296] leading-snug">
