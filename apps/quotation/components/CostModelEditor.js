  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-[1fr,340px] gap-4 items-start">
      ...
      <div className="glass-card overflow-hidden">
        <div className="flex flex-wrap border-b border-[#D9E4E6] dark:border-white/[0.08]">
          {SECTIONS.map(s => {
            ...
            return (
              <button key={s} type="button" onClick={() => setTab(s)}
                className={'px-3.5 py-2.5 text-sm transition-colors ' +
                  (tab === s ? 'border-b-2 border-brand-600 text-brand-700 dark:text-brand-300 font-medium' : 'text-[#7C9296] hover:text-inherit')}>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="text-[11px] uppercase tracking-wider text-[#7C9296]">
            ...
            <tbody>
              {sectionLines.length === 0 ? (
                <tr><td colSpan={9} className="px-3 py-8 text-center text-[#7C9296]">{t('cost.emptySection')}</td></tr>
              ) : sectionLines.map(l => (
                <tr key={l._i} className="border-t border-[#D9E4E6]/70 dark:border-white/[0.06] align-top">
                        ...
                        <div className="font-medium">
                          ...
                          {(l.section === 'material' || l.section === 'hardware') && (
                            <button type="button" title={t('matdlg.editMaterial')}
                              ...
                              className="ms-1.5 text-[#7C9296] hover:text-brand-600 dark:hover:text-brand-400 text-[13px] align-middle">✎</button>
                          )}
                        </div>
                        {l.spec_text && <div className="text-[11px] text-[#7C9296]" dir="ltr">{l.spec_text}</div>}
                  ...
                  <td className="px-2 py-2">
                    {l.section === 'labour' ? (
                      ...
                      <span className="text-[#7C9296]">{l.section === 'expense' ? t('expunit.' + (l.unit || 'fixed')) : (l.unit || '—')}</span>
                    )}
                  </td>
                  <td className="px-2 py-2">
                    ...
                    {SCALABLE_SECTIONS.includes(l.section) && (
                      <select
                        ...
                        className="mt-0.5 block w-24 text-[10px] bg-transparent text-[#7C9296] border border-[#D9E4E6] dark:border-white/[0.1] rounded px-1 py-0.5">
                  ...
                  <td className="px-2 py-2">
                    {(l.section === 'expense' && l.extra && l.extra.pct_of_production) ? (
                      <span className="text-[#7C9296]">—</span>
                    ) : (
            ...
            {sectionLines.length > 0 && (
              <tfoot><tr className="border-t border-[#D9E4E6] dark:border-white/[0.08]">
                <td colSpan={7} className="px-3 py-2 text-end text-[12px] text-[#7C9296]">{t('sec.' + tab)} — {t('cost.sectionTotal')}</td>
      ...
      <div className="glass-card p-4 xl:sticky xl:top-20 space-y-3">
        ...
        <div className="flex items-center justify-between gap-2 text-sm">
          <span className="text-[#7C9296]">{t('cost.overhead')}</span>
        ...
        <div className="flex items-center justify-between gap-2 text-sm">
          <span className="text-[#7C9296]">{t('cost.risk')}</span>
          <span className="flex items-center gap-2">
        ...
        <div className="border-t border-[#D9E4E6] dark:border-white/[0.08] pt-3 space-y-2">
          <div className="flex items-center justify-between gap-2 text-sm">
            <span className="text-[#7C9296]">{t('cost.profit')}</span>
            <span className="flex items-center gap-2">
          ...
          <div className="flex items-center justify-between gap-2 text-sm">
            <span className="text-[#7C9296]">{t('cost.rounding')}</span>
        ...
        <div className="rounded-xl bg-brand-600/10 border border-brand-600/25 p-3 space-y-1">
          ...
          <div className="flex justify-between text-[12px] text-[#7C9296]">
...
function Row({ label, value, strong, big }) {
  return (
    <div className={'flex items-center justify-between text-sm ' + (big ? 'text-base font-semibold' : '')}>
      <span className={strong ? 'font-medium' : 'text-[#7C9296]'}>{label}</span>
