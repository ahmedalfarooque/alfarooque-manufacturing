...
export function GlassSearch({ value, onChange, placeholder, className = '' }) {
  return (
    <div className={'relative ' + className}>
      <span className="pointer-events-none absolute inset-y-0 start-3 flex items-center text-[var(--tx-4)]"><GlassIcon name="chart" size={0} bare /><svg width="15" height="15" viewBox="0 0 24 24" fill="none"><circle cx="11" cy="11" r="7" stroke="currentColor" strokeWidth="1.6"/><path d="M20 20l-3.2-3.2" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round"/></svg></span>
...
export function GlassDrawer({ title, children, onClose, side = 'end' }) {
  ...
  const from = side === 'start' ? '-translate-x-full rtl:translate-x-full' : 'translate-x-full rtl:-translate-x-full';
  return (
    <div className="fixed inset-0 z-50">
      <div className="absolute inset-0 bg-[#04101a]/70 backdrop-blur-md" onClick={onClose} />
      <div className={'absolute inset-y-0 ' + pos + ' w-[min(30rem,calc(100vw-2rem))] glass-card !rounded-none !rounded-s-[24px] p-5 overflow-y-auto animate-[none] ' + from.replace('translate', 'translate')}
