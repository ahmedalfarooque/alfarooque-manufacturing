...
/* ── GlassToast — inline controlled toast banner (toast={kind,text} + onClose). ── */
const TOAST_STYLES = {
  ...
  info:    'bg-[color:var(--pr-soft)] text-[color:var(--pr)] border border-[color:var(--pr)]/30',
};
export function GlassToast({ toast, onClose }) {
  if (!toast) return null;
  const cls = TOAST_STYLES[toast.kind] || TOAST_STYLES.info;
  return (
    <div className={'fixed top-4 right-4 z-[200] flex items-start gap-3 rounded-lg px-4 py-3 text-sm max-w-md shadow-lg ' + cls} role="alert">
      <span className="flex-1">{toast.text}</span>
      {onClose && <button onClick={onClose} className="text-current opacity-60 hover:opacity-100 leading-none text-base">×</button>}
    </div>
  );
}
