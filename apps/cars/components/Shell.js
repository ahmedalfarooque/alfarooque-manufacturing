...
export default function Shell({ children, active }) {
  ...
  useEffect(() => {
    fetch('/cars/api/auth', { credentials: 'same-origin' })
  ...
  async function logout() {
    try { await fetch('/cars/api/auth', { method: 'POST', headers: { 'Content-Type': 'application/json' }, credentials: 'same-origin', body: JSON.stringify({ action: 'logout' }) }); } catch (_) {}
    ...
    <div className="min-h-screen flex bg-[#f3f5f7] dark:bg-[#0b1220] text-slate-900 dark:text-slate-100">
      ...
      <aside className={
        ...
        <nav className="flex-1 px-3 space-y-1 mt-2">
          {NAV.map(item => (
            <a key={item.href} href={'/cars' + item.href}
              className={
