...
export default function Shell({ children, active }) {
  ...
  return (
    <div className="min-h-screen flex text-[#122A30] dark:text-[#F4F9FA] transition-colors duration-300">
      {/* Sidebar */}
      <aside className={
        'af-shell-aside fixed lg:static z-40 inset-y-0 start-0 w-64 shrink-0 bg-[#EEF3F4]/85 dark:bg-[#0F2A36]/75 backdrop-blur-2xl backdrop-saturate-150 border-e border-[#D9E4E6] dark:border-white/10 text-[#41585C] dark:text-[#9DB3B6] flex flex-col transition-transform ' +
        (sidebarOpen ? 'translate-x-0' : '-translate-x-full rtl:translate-x-full lg:!translate-x-0')
      }>
        <div className="flex items-center gap-3 px-5 h-16 border-b border-[#D9E4E6] dark:border-white/10">
          <img src="/logo.png" alt="AL FAROOQUE" className="h-9 w-9 object-contain shrink-0" />
          <div>
            <div className="text-[#122A30] dark:text-[#F4F9FA] font-semibold text-sm leading-tight">TrackFleet</div>
            <div className="text-[11px] text-[#7C9296]">{t('shell.tagline')}</div>
          </div>
        </div>
        <div className="px-5 py-3 text-[11px] uppercase tracking-wider text-[#7C9296]">{user?.role ? trEnum(t, 'role', user.role) : ' '}</div>
        <div className="px-5 pb-3 flex items-center gap-2">
          <div className="h-8 w-8 rounded-full bg-[#0C93AE]/15 flex items-center justify-center text-xs font-medium text-[#076B80] dark:text-brand-300">
            {(user?.full_name || user?.email || '?').slice(0, 1).toUpperCase()}
          </div>
          <div className="min-w-0">
            <div className="text-sm text-[#122A30] dark:text-[#F4F9FA] truncate">{user?.full_name || t('shell.loading')}</div>
            <div className="text-[11px] text-[#7C9296] capitalize truncate">{user?.role ? trEnum(t, 'role', user.role) : ''}</div>
          </div>
        </div>
          ...
          {NAV.map(item => (
            <a key={item.href} href={item.href}
              className={
                ...
                (active === item.href ? 'af-nav-active' : 'hover:bg-[#0C93AE]/8 dark:hover:bg-white/5 text-[#41585C] dark:text-[#9DB3B6]')
              }>
        ...
        <div className="p-3">
          <button onClick={logout} className="w-full flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm text-[#41585C] dark:text-[#9DB3B6] hover:bg-[#0C93AE]/8 dark:hover:bg-white/5 transition">
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        <header className="af-shell-header h-16 flex items-center justify-between px-4 lg:px-6 border-b border-[#D9E4E6] dark:border-white/10 bg-white/75 dark:bg-[#0F2A36]/65 backdrop-blur-2xl backdrop-saturate-150 sticky top-0 z-20">
          <div className="flex items-center gap-3">
            <button className="lg:hidden text-xl" onClick={() => setSidebarOpen(true)} aria-label={t('shell.openMenu')}>☰</button>
          ...
          <div className="flex items-center gap-3 flex-1 max-w-md relative">
            <input
              ...
              className="w-full rounded-lg border border-[#D9E4E6] dark:border-white/10 bg-transparent px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-[#0C93AE]/40 focus:border-[#0C93AE]/40 transition"
            />
