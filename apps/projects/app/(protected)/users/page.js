...
const ROLE_LABEL = { admin: 'Admin', viewer: 'Internal Company User', external: 'External Assigned User' };
const ROLE_BADGE = {
    ...
    <Shell active="/users">
      <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
        <div>
          ...
          <p className="text-xs text-slate-500">Dashboard &gt; Users</p>
        </div>
        <button onClick={() => setModal({ mode: 'add', data: EMPTY_FORM })} className="text-sm px-3 py-2 rounded-lg bg-brand-500 text-white">+ Add User</button>
      </div>

      <div className="rounded-xl border border-black/5 dark:border-white/10 bg-white dark:bg-white/[0.03] p-4 mb-4">
        <input placeholder="Search users by name, email, position, department…" value={search} onChange={e => setSearch(e.target.value)}
      ...
      <div className="rounded-xl border border-black/5 dark:border-white/10 bg-white dark:bg-white/[0.03] overflow-auto max-h-[70vh]">
        <table className="w-full text-sm min-w-[950px]">
          <thead className="text-left text-slate-400 text-xs border-b border-black/5 dark:border-white/10 sticky top-0 z-10 bg-white dark:bg-[#0f172a]">
            <tr>
              <th onClick={() => toggleSort('full_name')} className="py-3 px-4 cursor-pointer select-none hover:text-slate-600 dark:hover:text-slate-200">Name<SortIndicator column="full_name" sortKey={sortKey} sortDir={sortDir} /></th>
              ...
              <th onClick={() => toggleSort('role')} className="cursor-pointer select-none hover:text-slate-600 dark:hover:text-slate-200">Role<SortIndicator column="role" sortKey={sortKey} sortDir={sortDir} /></th>
              <th>Position</th>
              <th>Department</th>
              <th>Company</th>
          ...
          <tbody>
            {!data ? (
              ...
              <tr><td colSpan={8} className="py-8 text-center text-slate-400">No users yet.</td></tr>
            ) : users.map(u => (
              <tr key={u.id} className="border-b border-black/5 dark:border-white/5">
                <td className="py-3 px-4 font-medium">{u.full_name}</td>
                <td>{u.email}</td>
                <td><span className={'px-2 py-0.5 rounded-full text-[11px] font-medium ' + (ROLE_BADGE[u.role] || '')}>{ROLE_LABEL[u.role] || u.role}</span></td>
                <td>{u.position || '—'}</td>
                <td>{u.department || '—'}</td>
                <td>{u.company || '—'}</td>
                <td><span className={'px-2 py-0.5 rounded-full text-[11px] font-medium ' + (STATUS_BADGE[u.status || 'Active'] || '')}>{u.status || 'Active'}</span></td>
                <td className="text-right px-4 space-x-2">
                  <button onClick={() => setModal({ mode: 'edit', data: u })} title="Edit" className="text-brand-500">✎</button>
                </td>
              </tr>
            ))}
          </tbody>
      ...
      <div className="flex items-center justify-between mt-4 text-sm text-slate-500 flex-wrap gap-3">
        <div className="flex items-center gap-3">
          <span>Showing {users.length ? (page - 1) * pageSize + 1 : 0} to {(page - 1) * pageSize + users.length} of {total} entries</span>
    ...
    return (
      <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
        <div className="w-full max-w-md rounded-2xl bg-white dark:bg-[#0f172a] p-6 space-y-4">
          ...
          <p className="text-sm text-slate-500">Share this one-time temporary password with {form.full_name} — it will not be shown again.</p>
          ...
          <div className="flex justify-end"><button onClick={onClose} className="px-4 py-2 rounded-lg bg-brand-500 text-white text-sm">Done</button></div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" onClick={onClose}>
      <form onSubmit={submit} onClick={e => e.stopPropagation()} className="w-full max-w-lg rounded-2xl bg-white dark:bg-[#0f172a] p-6 space-y-4 max-h-[90vh] overflow-y-auto">
        <h3 className="font-semibold text-lg">{modal.mode === 'add' ? 'Add User' : 'Edit User'}</h3>
        {err && <div className="text-red-500 text-sm">{err}</div>}
        <div className="grid grid-cols-2 gap-3">
          <Field label="Full Name" value={form.full_name} onChange={set('full_name')} required />
          <Field label="Email" type="email" value={form.email || ''} onChange={set('email')} disabled={modal.mode === 'edit'} required={modal.mode === 'add'} />
          <div>
            <label className="block text-xs text-slate-500 mb-1">Role</label>
            <Dropdown value={form.role} onChange={v => setForm(f => ({ ...f, role: v }))}
              options={[['viewer', 'Internal Company User'], ['external', 'External Assigned User'], ['admin', 'Admin']]} />
          </div>
          <div>
        ...
        {modal.mode === 'add' && form.role !== 'admin' && (
          <p className="text-xs text-slate-500">This user logs in via email + one-time code (OTP) only — no password is set.</p>
        )}
