...
function otpEmailHtml(code) {
  return '<div style="font-family:Arial,sans-serif;max-width:480px;margin:0 auto;padding:32px 24px;">' +
    '<h2 style="color:#0f877e;margin:0 0 12px;">ProTrack — Login Code</h2>' +
...
function viewOtpEmailHtml(code) {
  return '<div style="font-family:Arial,sans-serif;max-width:480px;margin:0 auto;padding:32px 24px;">' +
    '<h2 style="color:#0f877e;margin:0 0 12px;">ProTrack — View Dashboard Code</h2>' +
    '<p style="color:#333;font-size:14px;line-height:1.6;">Use this code to sign in to the read-only Project Management view. It expires in ' + OTP_TTL_MINUTES + ' minutes and can only be used once.</p>' +
    '<div style="font-size:32px;font-weight:700;letter-spacing:8px;background:#f2f2f2;padding:16px 24px;border-radius:8px;text-align:center;margin:20px 0;">' + code + '</div>' +
    '<p style="color:#888;font-size:12px;">If you did not request this, you can safely ignore this email.</p>' +
  '</div>';
}
...
export async function POST(req) {
  ...
  try {
    ...
    console.error('[projects/auth]', action, err);
...
async function handleLogin(sb, body, ip) {
  ...
  if (otpInsertErr) {
    console.error('[projects/auth] OTP insert failed:', otpInsertErr.message);
    return json({ error: 'Could not start verification. Please try again.' }, 500);
  }

  try {
    const result = await sendOtpEmail({ to: user.email, subject: 'Your ProTrack login code', html: otpEmailHtml(code), mockLabel: 'Projects login OTP', code });
    ...
    console.error('[projects/auth] OTP email failed:', err.message);
...
async function handleVerifyOtp(sb, body, ip, ua) {
  ...
  if (sessionInsertErr) {
    console.error('[projects/auth] session insert failed:', sessionInsertErr.message);
...
async function handleResendOtp(sb, body) {
  ...
  try {
    const result = await sendOtpEmail({ to: user.email, subject: 'Your ProTrack login code', html: otpEmailHtml(code), mockLabel: 'Projects resend OTP', code });
...
async function handleViewLogin(sb, body) {
  ...
  if (otpInsertErr) {
    console.error('[projects/auth] view-login OTP insert failed:', otpInsertErr.message);
    return json({ error: 'Could not start verification. Please try again.' }, 500);
  }

  try {
    const result = await sendOtpEmail({ to: email, subject: 'Your ProTrack View Dashboard code', html: viewOtpEmailHtml(code), mockLabel: 'Projects view-login OTP', code });
    ...
    console.error('[projects/auth] view-login OTP email failed:', err.message);
    return json({ error: 'Could not send the verification email. Please try again shortly.' }, 500);
  }
}
...
async function handleViewVerifyOtp(sb, body, ip, ua) {
  ...
  if (sessionInsertErr) {
    console.error('[projects/auth] view-login session insert failed:', sessionInsertErr.message);
    return json({ error: 'Could not complete sign-in. Please try again.' }, 500);
  }
  await sb.from('platform_users').update({ last_login_at: new Date().toISOString() }).eq('id', user.id);
...
async function handleViewResendOtp(sb, body) {
  ...
  try {
    const result = await sendOtpEmail({ to: email, subject: 'Your ProTrack View Dashboard code', html: viewOtpEmailHtml(code), mockLabel: 'Projects view-login resend OTP', code });
    return json({ ok: true, mocked: !!result.mocked, message: result.mocked ? 'Email not configured — new code was logged to the server console.' : 'A new code has been sent.' });
  } catch (err) {
    return json({ error: 'Could not send the verification email.' }, 500);
  }
