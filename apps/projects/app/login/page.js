...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[color:var(--bg)] px-4 transition-colors duration-200">
      <div className="w-full max-w-md glass-card p-5 sm:p-8 gfade-up">
        ...
        {step === 'credentials' && (
          <div className="grid grid-cols-2 gap-1.5 mb-6">
            <button type="button" onClick={() => switchMode('user')}
              className={'gbtn gbtn--sm gbtn--block ' + (mode === 'user' ? 'gbtn-primary' : 'gbtn-ghost')}>
              {t('login.user')}
            </button>
            <button type="button" onClick={() => switchMode('admin')}
              className={'gbtn gbtn--sm gbtn--block ' + (mode === 'admin' ? 'gbtn-primary' : 'gbtn-ghost')}>
              {t('login.admin')}
            </button>
          </div>
        )}
        ...
        {step === 'credentials' ? (
          <form onSubmit={submitCredentials} className="space-y-4">
            <div>
              <label className="block text-xs font-medium text-[color:var(--tx-3)] mb-1">{t('login.email')}</label>
              <input type="email" required value={email} onChange={e => setEmail(e.target.value)}
                className="ginput w-full" />
            </div>
            {mode === 'admin' && (
              <div>
                <label className="block text-xs font-medium text-[color:var(--tx-3)] mb-1">{t('login.password')}</label>
                <input type="password" required value={password} onChange={e => setPassword(e.target.value)}
                  className="ginput w-full" />
              </div>
            )}
            <button disabled={busy} type="submit"
              className="gbtn gbtn-primary gbtn--block">
          ...
          <form onSubmit={submitOtp} className="space-y-4">
            ...
            <input inputMode="numeric" pattern="[0-9]*" maxLength={6} required value={code}
              ...
              className="ginput w-full text-center tracking-[0.5em] text-lg" />
            <button disabled={busy} type="submit"
              className="gbtn gbtn-primary gbtn--block">
              {busy ? t('login.verifying') : t('login.verifyAndSignIn')}
            </button>
            <button type="button" disabled={cooldown > 0} onClick={resend}
              className="gbtn gbtn-secondary gbtn--block">
              {cooldown > 0 ? t('login.resendCodeCountdown', { seconds: cooldown }) : t('login.resendCode')}
            </button>
            <button type="button" onClick={() => switchMode(mode)}
              className="w-full text-center text-xs text-[color:var(--tx-3)] hover:text-[color:var(--tx-2)]">{mode === 'admin' ? t('login.backToEmailPassword') : t('login.backToEmail')}</button>
          </form>
        )}
      </div>
    </div>
