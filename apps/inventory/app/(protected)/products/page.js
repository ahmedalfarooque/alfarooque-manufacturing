          ...
          <table className="w-full text-sm">
            <thead className="border-b border-[color:var(--bd)] bg-[color:var(--bg-card)]">
              <tr>
                ...
                <th className="text-end px-4 py-3 font-medium text-[color:var(--tx-3)]">{t('products.sellingPrice')}</th>
            ...
            <tbody className="divide-y divide-[color:var(--bd)]">
              ...
              {products.map(p => (
                <tr key={p.id} className="hover:bg-[color:var(--pr-soft)] transition-colors">
                  <td className="px-4 py-3 font-medium">{p.name}</td>
                  <td className="px-4 py-3 text-[color:var(--tx-3)]">{p.sku || '—'}</td>
                  <td className="px-4 py-3 text-[color:var(--tx-3)]">{p.inv_categories?.name || '—'}</td>
                  <td className="px-4 py-3 text-end font-semibold">{Number(p.qty_on_hand || 0).toLocaleString()} <span className="text-[color:var(--tx-3)] font-normal text-xs">{p.inv_units?.symbol || ''}</span></td>
                  <td className="px-4 py-3 text-end">SAR {Number(p.selling_price || 0).toFixed(2)}</td>
        ...
        <GlassModal title={modal === 'add' ? t('products.addProduct') : t('products.editProduct')} onClose={closeModal}>
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              ...
              <GlassInput value={form.name || ''} onChange={v => setForm(f => ({ ...f, name: v }))} placeholder={t('products.namePlaceholder')} />
            </div>
            <div>
              <label className="block text-xs font-medium text-[color:var(--tx-3)] mb-1">{t('products.sku')}</label>
              <GlassInput value={form.sku || ''} onChange={v => setForm(f => ({ ...f, sku: v }))} placeholder="SKU-001" />
            </div>
            <div>
              <label className="block text-xs font-medium text-[color:var(--tx-3)] mb-1">{t('products.barcode')}</label>
              <GlassInput value={form.barcode || ''} onChange={v => setForm(f => ({ ...f, barcode: v }))} />
            </div>
            <div>
            ...
            <div>
              ...
              <GlassInput type="number" value={form.selling_price || ''} onChange={v => setForm(f => ({ ...f, selling_price: v }))} placeholder="0.00" />
            </div>
            <div>
            ...
            <div>
              ...
              <GlassInput type="number" value={form.max_stock_qty || ''} onChange={v => setForm(f => ({ ...f, max_stock_qty: v }))} placeholder="" />
            </div>
            <div className="col-span-2">
