...
/* POST /api/auth  { action: 'login' | 'verify-otp' | 'resend-otp' | 'logout' }
   ...
   Staff login: email + password → OTP → JWT session cookie.
   SSO cookie is minted for ALL authenticated users so any ERP staff
   member can switch apps seamlessly. */
...
function otpEmailHtml(code) {
  return '<div style="font-family:Arial,sans-serif;max-width:480px;margin:0 auto;padding:32px 24px;">' +
    '<h2 style="color:#06B6D4;margin:0 0 12px;">Inventory — Login Code</h2>' +
    '<p style="color:#333;font-size:14px;line-height:1.6;">Use this code to finish signing in to the AL FAROOQUE Inventory dashboard. It expires in ' + OTP_TTL_MINUTES + ' minutes and can only be used once.</p>' +
...
async function handleLogin(sb, body, ip) {
  ...
  if (otpInsertErr) {
    console.error('[inventory/auth] OTP insert failed:', otpInsertErr.message);
    return json({ error: 'Could not start verification. Please try again.' }, 500);
  }

  try {
    const result = await sendOtpEmail({ to: user.email, subject: 'Your Inventory login code', html: otpEmailHtml(code), mockLabel: 'Inventory login OTP', code });
    ...
    console.error('[inventory/auth] OTP email failed:', err.message);
...
async function handleVerifyOtp(sb, body, ip, ua, req) {
  ...
  if (sessionInsertErr) {
    console.error('[inventory/auth] session insert failed:', sessionInsertErr.message);
  ...
  /* SSO cookie minted for ALL authenticated users — any ERP staff member
     can switch apps without re-logging in. */
...
async function handleResendOtp(sb, body) {
  ...
  try {
    const result = await sendOtpEmail({ to: user.email, subject: 'Your Inventory login code', html: otpEmailHtml(code), mockLabel: 'Inventory resend OTP', code });
