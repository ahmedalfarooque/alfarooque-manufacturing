...
-- Version: inv-schema-v04-accounting-schema-fix
...
create table acc_journal_entries (
  ...
  created_at     timestamptz not null default now(),
  updated_at     timestamptz not null default now(),
  constraint acc_journal_entries_created_by_fkey foreign key (created_by) references platform_users(id)
);
...
create table if not exists acc_expenses (
  ...
  created_at      timestamptz not null default now(),
  updated_at      timestamptz not null default now(),
  constraint acc_expenses_created_by_fkey foreign key (created_by) references platform_users(id)
);
